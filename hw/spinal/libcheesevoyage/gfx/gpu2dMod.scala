...
case class Gpu2d(
  ...
  val loc = new Area {
    ...
    val rWrLineNum = KeepAttribute(
      Reg(UInt(log2Up(params.intnlFbSize2d.y) bits)) init(0x2)
    )
    ...
    val rWrLineNumCheckPipe2 = Reg(Bool()) init(False)
    val rWrLineNumPlus1Pipe2 = Reg(UInt(rWrLineNum.getWidth + 1 bits))
      .init(0x0)
    //val rWrLineNumPlus1Pipe2 := Reg(cloneOf(rWrLineNum)) init(0x0)
    val rWrLineNumPipe1 = Reg(UInt(rWrLineNum.getWidth + 1 bits)) init(0x0)
    rWrLineNumCheckPipe2 := (
      rWrLineNum.resized =/= params.intnlFbSize2d.y - 1
    )
    def tempWrLineNumPipeWidth = rWrLineNumPlus1Pipe2.getWidth
    //rWrLineNumPlus1Pipe2 := rWrLineNum.resized + 1
    rWrLineNumPlus1Pipe2 := (
      rWrLineNum.resized + U(f"$tempWrLineNumPipeWidth'd1")
    )
    when (rWrLineNumCheckPipe2) {
      rWrLineNumPipe1 := rWrLineNumPlus1Pipe2
    } otherwise {
      rWrLineNumPipe1 := 0
    }
    when (
      ...
      //when (rWrLineNum + 1 < params.intnlFbSize2d.y) {
    ...
    def doInitWrBgPipePayload(): WrBgPipePayload = {
    ...
    def doInitWrObjPipePayload(): WrObjPipePayload = {
    ...
    def doInitCombinePipePayload(): CombinePipePayload = {
    ...
    def doInitRdPipePayload(): RdPipePayload = {
    ...
    when (rIntnlChangingRow) {
      ...
      when (wrObjPipeIn(0).fire) {
        rWrObjPipeFrontPayload.bakCnt := rWrObjPipeFrontPayload.bakCnt - 1
      ...
      //  rWrObjPipeFrontValid := False
      //}
      when (rWrObjPipeFrontPayload.bakCntWillBeDone()) {
        rWrObjPipeFrontValid := False
    ...
    rRdPipeFrontPayload.init(doInitRdPipePayload())
    ...
    //rdPipeLastOut.translateInto

    //rdPipeLastOutPop.payload := outp
    //rdPipeLastOut.payload := outp

    //rdPipeLastIn.ready := True
    //rdPipeLastOut.valid := rdPipeLastIn.valid
    //rdPipeLastOut.payload := rdPipeLastIn.payload
    //rdPipeLastOut.payload.col := rdPipeLastIn.
    //rdPipeLastOut.payload := outp

    //rdPipeLastOut.ready := pop.ready
    ////pop.valid := rdPipeLastOut.valid
    //pop.payload := rdPipeLastOut.payload

    //rdPipeLastOut.ready := pop.ready
    //pop.valid := 
    //pop.payload

    //pop <-/< rdPipeLastOutPop

    //rdPipeLastOut.payload
    //rdPipeLastIn.ready := True

    //rdPipeLastOut.payload 
    //rdPipeLastIn.ready := rdPipeLastOut.
            ...
            for (tempBgIdx <- 0 to params.numBgs - 1) {
              is (tempBgIdx) {
                ...
                def tempSliceRange = ElabVec2(
                  ...
                  x=(
                    //(
                    //  //params.bgEntryMemIdxWidth + params.bgTileSize2dPow.x 
                    //  //- 1
                    //  0
                    //)
                    //inpScroll.x.high - 1
    ...
    def readLineMemEntries(
      ...
      HandleDualPipe(
        ...
        pipeStageMainFunc=(
          ...
          switch (rRdLineMemArrIdx) {
            for (jdx <- 0 to (1 << rRdLineMemArrIdx.getWidth) - 1) {
              is (jdx) {
