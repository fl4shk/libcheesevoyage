...
case class Gpu2d(
  ...
  val loc = new Area {
    ...
    for (idx <- 0 until params.numObjMemsPerKind(isTileMem=true)) {
      ...
      objTileMemArr(idx).io.wrData := tempObjTilePush.payload.tileSlice
      ...
      //) => 
      {
        ...
        def myMainFunc(
          ...
          val tempOverwriteLineMemEntry = (
            ...
            .setName(
              f"wrObjPipe14_tempOverwriteLineMemEntry_$kind" + f"$x"
            )
          //--------
          // BEGIN: later
          ...
          val tempRdLineMemEntry = ObjSubLineMemEntry()
            .setName(f"wrObjPipe14_tempRdLineMemEntry_$kind" + f"_$x")
