...
case class LcvSimpleReorderBufConfig[
  ...
  wordType: HardType[WordT],
  reorderBufIdxWidth: Int,
  //wrAutoIncr: Boolean=true,
) {
  val depth = (1 << reorderBufIdxWidth)
  //def myPushType() = (
  //  PipeSimpleDualPortMemDrivePayload(
  //    dataType=modType(),
  //    wordCount=depth,
...
case class LcvSimpleReorderBufPushPayload[
  WordT <: Data
](
  cfg: LcvSimpleReorderBufConfig[WordT]
) extends Bundle {
  val reorderBufIdx = UInt(cfg.reorderBufIdxWidth bits)
  val myWord = cfg.wordType()
}

case class LcvSimpleReorderBufIo[
  WordT <: Data,
](
  //cfg: WrPulseRdPipeRamConfig[WordT]
  cfg: LcvSimpleReorderBufConfig[WordT]
) extends Bundle {
  val push = slave(Stream(
    //PipeSimpleDualPortMemDrivePayload(
    ...
    //)
    //cfg.myPushType()
    LcvSimpleReorderBufPushPayload(cfg=cfg)
  ))
  val pop = master(Stream(
    cfg.wordType()
  ))
...
case class LcvSimpleReorderBuf[
  WordT <: Data,
](
  cfg: LcvSimpleReorderBufConfig[WordT]
) extends Component {
  //--------
  val io = LcvSimpleReorderBufIo(cfg=cfg)
  //--------
  val myRam = (
    WrPulseRdPipeRam(
      cfg=WrPulseRdPipeRamConfig(
        modType=LcvSimpleReorderBufPushPayload(cfg=cfg),
        wordType=cfg.wordType(),
        wordCount=cfg.depth,
        setWordFunc=(
          outp: LcvSimpleReorderBufPushPayload[WordT],
          inp: LcvSimpleReorderBufPushPayload[WordT],
          rdMemWord: WordT,
          upIsFiring: Bool,
          myExternalInpCond: Bool,
          wrPulseVec: Vec[Flow[
            PipeSimpleDualPortMemDrivePayload[
              //PipeSimpleDualPortMemDrivePayload[ModT]
              WordT
            ]
          ]],
        ) => {
            ...
            is (M"01") {
              ...
              outp.myWord := (
                ...
                RegNextWhen(
                  ...
                  init=wrPulseVec.head.data.getZero
                )
              )
            }
            default {
              outp.myWord := rdMemWord
            }
          }
        },
        optRdLatency=(
        ...
        initBigInt=Some({
          ...
          tempArr ++= Array.fill(cfg.depth)(BigInt(0))
          Array(tempArr).toSeq
        }),
        arrRamStyleAltera=(
          //"no_rw_check, logic"
  ...
  val myAssertValidCond = myRam.io.wrPulse.fire
  myRam.io.myExternalInpCond := True
  ...
  io.occupancy := (
    //RegNext(myOccupancy, init=myOccupancy.getZero)
    myOccupancy
  )
  ...
  myRam.io.wrPulse.data := io.push.myWord
  when (myAssertValidCond) {
  ...
  myRam.io.rdAddrPipe.valid := rValidVec(myRdAddr)
...
case class LcvSimpleStreamFifoIo[
  WordT <: Data
](
  cfg: LcvSimpleReorderBufConfig[WordT]
) extends Bundle {
...
case class LcvSimpleStreamFifo[
  ...
  val io = LcvSimpleStreamFifoIo(cfg=cfg)
  val myReorderBuf = LcvSimpleReorderBuf(cfg=cfg)
  io.push.translateInto(myReorderBuf.io.push)(
    dataAssignment=(outp, inp) => {
      outp.myWord := inp
      outp.reorderBufIdx := (
        (
          RegNextWhen(
            (outp.reorderBufIdx + 1),//.asSInt,
            cond=myReorderBuf.io.push.fire,
          )
          init(
            //-1
            0x0
  ...
  io.pop << myReorderBuf.io.pop
  io.occupancy <> myReorderBuf.io.occupancy
}
  ...
  val cFrontArea = new cFront.Area {
    ...
    if (cfg.optRdLatency == 0) {
      cfg.setWordFunc(
        ...
        Vec(io.wrPulse, io.otherWrPulse),
      )
...
private[libcheesevoyage] case class WrPulseRdPipeRamSimpleDualPort[
  ...
  //ram.io.rdEn := True
  //ram.io.rdAddr := (
  //  RegNext(ram.io.rdAddr, init=ram.io.rdAddr.getZero)
  //)
  case class MainPayload(
  ) extends Bundle {
    val myInpPayload = cloneOf(io.rdAddrPipe.payload)
...
// 3 1000
  ...
  //    )
  //  }
  //  s2mLastBackArr += S2MLink(
  //    up=sLastBackArr.last.down,
  //    down={
  //      val temp = Node()
  //      temp.setName(s"s2mLastBackArr_${idx}_down")
  //      temp
  //    },
  //  )
  //}
...
private[libcheesevoyage] case class WrPulseRdPipeRamSdpPipe[
  ...
  val ram = RamSdpPipe(cfg=ramCfg)
  ram.io.wrEn := io.wrPulse.fire
  ram.io.wrAddr := io.wrPulse.addr
  ram.io.wrData := io.wrPulse.data
...
// 3 1000
  ...
  val cFrontArea = new cFront.Area {
    ...
    when (
      ...
      ram.io.rdAddr := up(mainPayload).myInpPayload.addr
    }
  }
  //--------
