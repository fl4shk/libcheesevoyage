...
case class LcvBusCacheBusPairConfig(
  ...
  val myFifoThingLoBusCfg = LcvBusConfig(
    mainCfg=mainCfg.mkCopyWithTxnCnt(LcvBusDoStallFifoThing.myCntWidth),
    cacheCfg=Some(loBusCacheCfg),
  )
...
//case class LcvBusCachePrefetcherConfig(
//  innerCfg: LcvBusCacheBusPairConfig,
//  numLinesAhead: Int=4,
//) {
//  require(
//    innerCfg.havePrefetch
//  )
//  require(
//    numLinesAhead >= 1
//  )
...
//}
//
//case class LcvBusCachePrefetcherHaveHitPayload(
//  //busCfg: LcvBusConfig
...
//  )
//
//  val haveHitAtAll = Bool()
//  //val addr = UInt(busCfg.addrWidth bits)
//  val loH2dPayload = LcvBusH2dPayload(cfg=cfg.loBusCfg)
//  def addr = loH2dPayload.addr
//}

//case class LcvBusCachePrefetcherIo(
//  cfg: LcvBusCachePrefetcherConfig
//) extends Bundle {
//  val loBus = slave(LcvBusIo(cfg=cfg.busCfg))
//  val hiBus = master(LcvBusIo(cfg=cfg.busCfg))
//
//  val haveHit = (
//    slave(Flow(
//      LcvBusCachePrefetcherHaveHitPayload(busCfg=cfg.busCfg)
//    ))
//  )
//}
//
//case class LcvBusCachePrefetcher(
//  cfg: LcvBusCachePrefetcherConfig
//) extends Component {
//  //--------
//  val io = LcvBusCachePrefetcherIo(cfg=cfg)
//  //--------
//  //val rPrevHaveHit = RegNext(io.haveHit, init=io.haveHit.getZero)
//  val rSavedHaveHit = {
//    val temp = Reg(Flow(LcvBusCachePrefetcherHaveHitPayload(
//      busCfg=cfg.busCfg
//    )))
//    //temp.valid.init(False)
//    //temp.payload.init(temp.payload.getZero)
//    temp.init(temp.getZero)
//    temp
//  }
//  rSavedHaveHit.addr.allowOverride
//
...
//  ) {
//    is (M"10") {
//      rCpuRealTxnCnt := rCpuRealTxnCnt + 1
//    }
//    is (M"01") {
//      rCpuRealTxnCnt := rCpuRealTxnCnt - 1
//    }
...
//    //is (M"11")
//    default {
//      // if they're the same, there should be no change!
//    }
//  }
//
//  when (
...
//    && io.hiBus.h2dBus.fire
//  ) {
//    rPrefetchH2dCnt := rPrefetchH2dCnt - 1
//    rSavedHaveHit.addr := (
...
//    && io.hiBus.d2hBus.fire
//  ) {
//    rPrefetchD2hCnt := rPrefetchD2hCnt - 1
//  }
//
//  switch (
//    rSavedHaveHit.fire
//    ## rSavedHaveHit.haveHitAtAll
//    ## rCpuRealTxnCnt.orR // check for non-zero
//  ) {
//    is (M"100") {
//      // There was a cache miss, so let's try fetching more than just one
//      // line!
//      io.hiBus.h2dBus.valid := !rPrefetchH2dCnt.msb
//      io.hiBus.h2dBus.addr := rSavedHaveHit.addr
//      io.hiBus.d2hBus.ready := True//!rPrefetchD2hCnt.msb
//        // we need to ignore the bus responses from prefetch requests, and
//        // not forward them back to the bus host!
//        // however, can we just leave
//    }
//    is (M"101") {
//      // This is the case when we saw there was a cache miss, but we can't
//      // prefetch until the cache is done responding to actual requests
//      // from the PCU.
...
case class LcvBusDoStallFifoThingIo(
  ...
  require(
    busCfg.optTxnCntWidth != None
  )
  require(
    busCfg.optTxnCntWidth.get == LcvBusDoStallFifoThing.myCntWidth
  )
  ...
  //val doStallNotYetD2hFire = doStall.last
  val doStall = in(Bool())
  //val doRepeatLastPop = in(Bool())
  val doInit = (
    includeDoInit
  ) generate (
    in(Bool())
...
case class LcvBusDoStallFifoThing(
  ...
  subFifoArr.foreach(item => {
    item.io.push.valid := False
    item.io.push.payload := item.io.push.payload.getZero
    item.io.pop.ready := False
    item.io.flush := False
  })

  //val myPushStm = StreamArbiterFactory.lowerFirst.onArgs(
  //  io.altPush,
  //  io.push,
  //)
  def myPushStm = io.push
  ...
  val myAlmostFinalPopStm = cloneOf(io.pop) //cloneOf(myPopStm)
  //when (io.doRepeatLastPop) {
  //  io.pop << myAlmostFinalPopStm.repeat(2)._1
  //} otherwise {
  //  io.pop << myAlmostFinalPopStm
  //}
  io.pop << myAlmostFinalPopStm
  ...
  if (!busCfg.haveByteEn) {
    ...
    myLoH2dToWrByteEnStmAdapter.io.hiH2dBus.translateInto(
      ...
      dataAssignment=(outp, inp) => {
        ...
        outp.busPayload.txnCnt := myPopStm.busPayload.txnCnt
        //outp.cnt := myPopStm.cnt
      }
    )
  } else {
    //io.pop << myPopStm
    myAlmostFinalPopStm << myPopStm
  }
  ...
  switch (rState) {
    ...
    is (State.IDLE) {
      switch (
        ...
        is (
          ...
          when (subFifoArr(0).io.push.fire) {
            when (!rFifoCntSub(0).msb) {
              rFifoCntSub(0) := rFifoCntSub(0) - 1
            } otherwise {
              subFifoArr.foreach(item => {
                item.io.pop.ready := True
              })
            }
          }
        }
    ...
    //  rState := State.POST_DO_STALL
    //}
    is (State.POST_DO_STALL_0) {
      rFifoCntSub(0) := fifoCntSubMax
      
      when (rose(rState === State.POST_DO_STALL_0)) {
        doApplyMainFifo(
          func=(mainFifo) => {
      ...
      //}
      //--------
      // BEGIN: old, mostly working code!
      when (
        !subFifoArr(0).io.pop.valid
        && !io.doStall
      ) {
        rState := State.IDLE
    ...
    is (State.POST_DO_STALL_1) {
      when (rose(rState === State.POST_DO_STALL_1)) {
...
case class LcvBusDoStallH2dThrowThing(
  ...
  val tempElemFoundBasicVec = Vec[Bool](
    (
      rSavedIoPushH2dThrowVec.head.fire
      && io.push.payload === rSavedIoPushH2dThrowVec.head.payload
    ),
    (
      rSavedIoPushH2dThrowVec.last.fire
      && io.push.payload === rSavedIoPushH2dThrowVec.last.payload
    )
  )
  ...
  val rPrevRewriteIdx = (
    Reg(UInt(log2Up(rSavedIoPushH2dThrowVec.size) bits))
    init(0x0)
  )
  ...
  switch (
    ...
    is (B"100") {
      when (rPrevRewriteIdx.lsb) {
        rSavedIoPushH2dThrowVec.head := io.push
        rPrevRewriteIdx.lsb := False
      } otherwise {
        rSavedIoPushH2dThrowVec.last := io.push
        rPrevRewriteIdx.lsb := True
      }
    }
    is (M"11-") {
      rSavedIoPushH2dThrowVec.last := io.push
      rSavedIoPushH2dThrowVec.head.valid := False
    }
    is (B"101") {
      rSavedIoPushH2dThrowVec.head := io.push
      rSavedIoPushH2dThrowVec.last.valid := False
    }
    default {
    }
  }
...
case class LcvBusDoStallH2dReptThingPayload(
  busCfg: LcvBusConfig,
  optIncludeSavedData: Boolean,
) extends Bundle {
  val busPayload = LcvBusH2dPayload(cfg=busCfg)

  def addr = busPayload.addr
  def data = busPayload.data
  def src = busPayload.src
  def isWrite = busPayload.isWrite
  def byteSize = busPayload.byteSize
  def byteEn = busPayload.byteEn

  def mainNonBurstInfo = busPayload.mainNonBurstInfo
  def infoByteSizeEtc = mainNonBurstInfo.infoByteSizeEtc
  def txnCnt = busPayload.txnCnt

  def burstAddr(
    someBurstCnt: UInt,
    incrBurstCnt: Boolean,
  ) = busPayload.burstAddr(
...
case class LcvBusDoStallH2dReptThingIo(
  busCfg: LcvBusConfig,
  optIncludeSavedData: Boolean,
) extends Bundle {
  val push = slave(Stream(
    //LcvBusH2dPayload(cfg=busCfg)
    LcvBusDoStallH2dReptThingPayload(
      busCfg=busCfg,
      optIncludeSavedData=optIncludeSavedData,
    )
  ))
...
case class LcvBusDoStallH2dReptThing(
  busCfg: LcvBusConfig,
  optIncludeSavedData: Boolean=false,
) extends Component {
  //--------
  val io = LcvBusDoStallH2dReptThingIo(
    busCfg=busCfg,
    optIncludeSavedData=optIncludeSavedData,
  )
  //--------
...
case class LcvBusCacheIo(
  ...
  //) generate (
  //  master(Flow(
  //    //Vec.fill(cfg.loBusCacheCfg.numWays)(
  //    //  Bool()
  //    //)
  //    LcvBusCachePrefetcherHaveHitPayload(busCfg=cfg.loBusCfg)
  //  ))
  //)
...
//  cfg: LcvBusCacheBusPairConfig
//) extends Bundle {
//  val rdLineWord = UInt(cfg.loBusCacheCfg.wordWidth bits)
//  val rdLineAttrs = LcvBusCacheLineAttrs(cfg=cfg.loBusCfg)
//}
...
//      dataType=(
//        //UInt(busCfg.dataWidth bits)
//        cloneOf(io.loBus.d2hBus.payload)
//      ),
//      depth=(loBusCfg.maxBurstSizeMinus1 + 1),
//      latency=(
//        cfg.busD2hFifoLatency
//      ),
//      forFMax=true,
//    )
//  )
...
//  }
//  io.loBus.d2hBus << myLoD2hFifo.io.pop
//
//  //val temp
...
//    cloneOf(myLoH2dReptThing.io.pop)
//  )
//  mySelLoH2dPopStm << myLoH2dReptThing.io.pop
//  //val mySelLoH2dPop
...
//  //  val myLoH2dThrowCond = Bool()
//  //  val myTempLoH2dPopStm = myLoH2dDoStallFifoThing.io.pop.throwWhen(
//  //    myLoH2dThrowCond
//  //  )
//  //  myLoH2dThrowCond := False
//  //  //myLoH2dPopStm.ready := False
//  //  myLoH2dPopStm << myTempLoH2dPopStm
//  //}
//  val rLoH2dPayload = (
//    RegNextWhen(
//      mySelLoH2dPopStm.payload,
//      cond=mySelLoH2dPopStm.fire,
//      init=mySelLoH2dPopStm.payload.getZero,
//    )
//  )
//  def rBusAddr = rLoH2dPayload.addr
//  val rDel2LoH2dPayload = (
//    RegNext/*When*/(
//      next=rLoH2dPayload,
...
//    )
//  )
//  def rDel2BusAddr = rDel2LoH2dPayload.addr
//
//  val rSavedLoH2dPayload = (
//    Reg(cloneOf(rLoH2dPayload))
//    init(rLoH2dPayload.getZero)
...
//    depth=(myFifoThingLoBusCfg.maxBurstSizeMinus1 + 1),
//    latency=cfg.busD2hFifoLatency,
//    forFMax=true,
//  )
//
//  myLoD2hFifo.io.pop.translateInto(io.loBus.d2hBus)(
//    dataAssignment=(outp, inp) => {
//      outp.mainNonBurstInfo := inp.mainNonBurstInfo
//    }
//  )
//  //val myLoD2hAlmostDonePushStm = cloneOf(myLoD2hPushStm)
//  //myLoD2hAlmostDonePushStm << myLoD2hPushStm
//  //myLoD2hAlmostDonePushStm << myLoD2hPushStm.throwWhen(
//  //  myLoD2hPushStm.busPayload.txnCnt.asSInt
//  //  === (
//  //    RegNextWhen(
//  //      myLoD2hPushStm.busPayload.txnCnt.asSInt,
//  //      cond=myLoD2hPushStm.fire,
//  //    )
//  //    init(1)
//  //  )
//  //)
//
//  // NOTE: this was previously `myLoD2hAlmostDonePushStm.translateInto`
//  myLoD2hPushStm.translateInto(
...
//  def doLineAttrsRamWrite(
//    busAddr: UInt,
//    lineAttrs: LcvBusCacheLineAttrs,
//    //byteEn: Option[UInt],
//    setEn: Boolean=true,
//  ): Unit = {
//    if (setEn) {
...
//    //byteEn match {
//    //  case Some(byteEn) => {
//    //    lineAttrsRam.io.wrByteEn := byteEn.asBits
//    //  }
//    //  case None => {
//    //    lineAttrsRam.io.wrByteEn := (
//    //      B(lineAttrsRam.io.wrByteEn.getWidth bits, default -> True)
//    //    )
//    //  }
//    //}
//  }
...
//  )
//  //--------
//  def myLoD2hPayload = myLoD2hPushStm.busPayload
//  //--------
//
...
//  //  //    myLoH2dPopStm.fire
//  //  //    //myD2hPushStm.fire
//  //  //    //myLoH2dDoStallFifoThing.io.pop.fire
//  //  //    //&& !myLoH2dPopThrowArea.myLoH2dThrowCond
//  //  //  ),
//  //  //  length=(
//  //  //    //2
...
//  //    myLoH2dPopThrowArea.myLoH2dThrowCond := True
//  //  }
//  //}
//
...
//    RegNext(
//      lineWordRam.io.wrEn
//      || RegNext(lineWordRam.io.wrEn, init=False)
...
//private[libcheesevoyage] case class LcvBusInstrCacheNoPrefetch(
//  cfg: LcvBusCacheBusPairConfig,
//) extends Component {
//  //--------
//  require(
...
//
//  def loBusCacheCfg = cfg.loBusCacheCfg
//  def myLineWordRamSingleWordAddrRshift = (
//    loBusCacheCfg.myLineWordRamSingleWordAddrRshift
//  )
//  def myLineAttrsRamAddrRshift = loBusCacheCfg.myLineAttrsRamAddrRshift
//  def wordWidth = loBusCacheCfg.wordWidth
//  def depthWords = loBusCacheCfg.depthWords
...
//    arrRamStyleXilinx=cfg.loBusCacheCfg.lineAttrsMemRamStyleXilinx,
//  )
//  val lineAttrsRam = Array.fill(numWays)(
//    RamSdpPipe(cfg=lineAttrsRamCfg)
//  )
//
//  val rdLineWord = Vec.fill(numWays)(
//    UInt(wordWidth bits)
//  )
//  val rdLineAttrs = Vec.fill(numWays)(
//    LcvBusCacheLineAttrs(cfg=loBusCfg)
//  )
//  for (idx <- 0 until numWays) {
//    rdLineWord(idx) := lineWordRam(idx).io.rdData
//    rdLineAttrs(idx) := lineAttrsRam(idx).io.rdData
//    lineWordRam(idx).io.wrEn := False
//    lineAttrsRam(idx).io.wrEn := False
//  }
//
//  val wrLineAttrs = LcvBusCacheLineAttrs(cfg=loBusCfg)
//  wrLineAttrs := RegNext(wrLineAttrs, init=wrLineAttrs.getZero)
...
//  //myMainLoH2dPopStm.ready := !myFifoThingDoStall
//  //myLoH2dReptThing.io.push << myMainLoH2dPopStm
//  myMainLoH2dPopStm.translateInto(
//    myLoH2dReptThing.io.push
//  )(
//    dataAssignment=(outp, inp) => {
//      outp.busPayload := inp
...
//    }
//    lineAttrsRam(ramIdx).io.wrAddr := (
//      (busAddr(busAddr.high downto myLineAttrsRamAddrRshift))
//      .resize(lineAttrsRam(ramIdx).io.wrAddr.getWidth)
//    )
...
//  )
//
//  val myTempHaveHitCmpEqLeft = Vec[UInt](
//    rdLineAttrs.map(item => item.tag)
//  )
//  val myTempHaveHitCmpEqRight = Vec.fill(numWays)(
//    RegNext(
//      RegNext(mySelLoH2dPopPayload.addr(loBusCacheCfg.tagRange))
//      init(0x0)
//    )
...
//      === myTempHaveHitCmpEqRight(ramIdx)
//    )
//    haveHit(ramIdx) := (
//      rdLineAttrs(ramIdx).fire
//      && tempHaveHitCmpEq(ramIdx)
//    )
//  }
//  //val haveHit = (
...
//    //Vec.fill(numWays)(
//      Reg(
//        cloneOf(rdLineAttrs.head.tag),
//        init=rdLineAttrs.head.tag.getZero,
//      )
//    //)
//  )
//
...
//    //## (
//    //  (
//    //    rdLineAttrs.map(item => item.dirty).orR
//    //  )
//    //  //|| (
//    //  //  //rdLine
//    //  //  RegNext(
...
//  wrLineAttrs.valid := True
//}
private[libcheesevoyage] case class LcvBusInstrCacheMain(
  cfg: LcvBusCacheBusPairConfig,
) extends Component {
  require(
    cfg.havePrefetch
  ...
  for (outerRamIdx <- 0 until 2) {
    for (ramIdx <- 0 until numWays) {
      ...
      if (outerRamIdx == 1) {
        myTempHaveHitCmpEqRight(outerRamIdx)(ramIdx) := (
          rSavedPrefetchLoH2dPayload.addr(loBusCacheCfg.tagRange)
        )
      }
    }
  }
  ...
  val tempToSwitchVec = Vec[Bits](
    ...
    (
      tempToSwitchNonHaveHitVec.last
      ## haveHit.last
    ),
  )
  //println(
  ...
  //myPrefetchHaveHitVec.head.haveHitAtAll := (
  //  haveHit.head.orR
  //  //haveHit.last.orR
  //)
  //myPrefetchHaveHitVec.last.haveHitAtAll := (
  //  haveHit.last.orR
  //  //haveHit.last.orR
  //)
  //myPrefetchHaveHitVec.foreach(item => {
  ...
  def doLineAttrsRamReadSync(
    ...
    lineAttrsRam(outerRamIdx).foreach(item => {
      if (setEn == 1) {
        item.io.rdEn := True
      } else if (setEn == 2) {
        item.io.rdEn := (
  ...
  def doLineBitPlruRamReadSync(
    ...
    if (setEn == 1) {
      lineBitPlruRam(outerRamIdx).io.rdEn := True
    } else if (setEn == 2) {
      lineBitPlruRam(outerRamIdx).io.rdEn := (
        RegNext(
          next=(
            //mySelLoH2dPopStm.valid
            mySelLoH2dPopStm.fire
    ...
    lineBitPlruRam(outerRamIdx).io.rdAddr := {
      //println(
      //  s"test info: busAddr("
      //  + s"${busAddr.high} downto ${myLineAttrsRamAddrRshift}"
      //  + s")"
      ...
      (
        ...
        //  busAddr(busAddr.high downto myLineAttrsRamAddrRshift)
        //)
        //.resize(lineBitPlruRam.head.io.rdAddr.getWidth)
        convBusAddrToLineIdx(
          someRam=lineBitPlruRam(outerRamIdx),
          busAddr=busAddr,
        )
      )
    }
  ...
  def doLineAttrsRamWrite(
    ...
    lineAttrsRam.foreach(item => {
      ...
      item(ramIdx).io.wrAddr := (
        //(busAddr(busAddr.high downto myLineAttrsRamAddrRshift))
        //.resize(item(ramIdx).io.wrAddr.getWidth)
        convBusAddrToLineIdx(
          someRam=item(ramIdx),
          busAddr=busAddr,
        )
      )
      item(ramIdx).io.wrData := lineAttrs
    })
  }
  def doLineBitPlruRamWrite(
    busAddr: UInt,
    ...
    lineBitPlruRam.foreach(item => {
      if (setEn) {
        item.io.wrEn := True
      }
      item.io.wrAddr := (
        //(busAddr(busAddr.high downto myLineAttrsRamAddrRshift))
        //.resize(item.io.wrAddr.getWidth)
        convBusAddrToLineIdx(
          someRam=item,
          busAddr=busAddr,
        )
      )
      item.io.wrData := lineBitPlru
    })
  }
...
// 3 0---
  ...
  def doWriteBitPlruRamDuringMiss(
    ...
    //wrLineBitPlru := rdLineBitPlru + 1
    if (busAddr != None) {
      wrLineBitPlru := someRdLineBitPlru//rdLineBitPlru
    }
    lineBitPlruRam.foreach(item => {
      //item.io.wrEn := True
      busAddr match {
        case Some(myBusAddr) => {
          item.io.wrEn := True
          item.io.wrAddr := convBusAddrToLineIdx(
            someRam=item,
            busAddr=myBusAddr,
          )
        }
  ...
  def doWriteBitPlruRamDuringHit(
    someRdLineBitPlru: UInt,
    ramIdx: Int,
    busAddr: Option[UInt]=None,
  ): Unit = {
    ...
    lineBitPlruRam.foreach(item => {
      // NOTE:
      // With the current state of this code,
      // `rLoState`'s handling of cache hits can (rarely) have the
      // `lineBitPlruRam` write port be overwritten by the prefetcher!
      // It will lose a little performance, potentially,
      // but hopefully not by that much!
      item.io.wrEn := True
      busAddr match {
        case Some(myBusAddr) => {
          item.io.wrAddr := convBusAddrToLineIdx(
...
//  //)
//
//  val myPrefetchLoH2dPopStm = (
//    cloneOf(myLoH2dReptThing.io.pop)
//  )
//
//  val mySelLoH2dPopStm = (
//    //cloneOf(myLoH2dReptThing.io.pop)
//    StreamArbiterFactory.lowerFirst.onArgs(
//      myPrefetchLoH2dPopStm,
//      myLoH2dReptThing.io.pop,
//    )
//  )
//
//  val rSavedLoH2dPayload = (
...
//    ))
//    init(
//      //rLoH2dPayload.getZero
//      mySelLoH2dPopStm.payload.getZero
//    )
//  )
//  def rSavedLoBusAddr = rSavedLoH2dPayload.addr
//  def rSavedLoBusAddrSet = rSavedLoBusAddr(loBusCacheCfg.setRange)
//
//  val rPrefetchCnt = {
//    // prefetch a few extra lines upon a miss!
//    val temp = Reg(Flow(SInt(log2Up(2) + 1 bits)))
//    temp.valid.init(False)
//    temp.payload.init(-1)
//    temp
//  }
//  myPrefetchLoH2dPopStm.valid := rPrefetchCnt.fire//!rPrefetchCnt.msb
//  myPrefetchLoH2dPopStm.payload.setAsReg()
//  //:= (
//  //  //RegNextWhen(
//  //  //  myLoH2dReptThing.io.pop.payload
//  //  //)
//  //  RegNext(
//  //    myPrefetchLoH2dPopStm.payload,
//  //    init=myPrefetchLoH2dPopStm.payload.getZero
//  //  )
//  //)
//  when (myPrefetchLoH2dPopStm.fire) {
//    rPrefetchCnt.payload := rPrefetchCnt.payload - 1
//    myPrefetchLoH2dPopStm.addr := {
//      //myPrefetchLoH2dPopStm.busPayload.burstAddr(
//      //  someBurstCnt=U(s"${loBusCfg.burstCntWidth}'d0"),
//      //  incrBurstCnt=false,
//      //) + 1
//      val myPrefetchAddr = (
//        myPrefetchLoH2dPopStm.busPayload.prefetchAddr()
//      )
//      Cat(
//        (myPrefetchAddr._2 + 1),
//        U(s"${cfg.loBusCfg.prefetchAddrIncrStartIdx}'d0"),
//      ).asUInt
//    }
//  }
//
//  //switch (
//  //  myPrefetchLoH2dPopStm.fire
//  //  ## rPrefetchCnt.msb
//  //) {
//  //  is (M"11") {
//  //  }
//  //  is (M"10") {
//  //    myPrefetchLoH2dPopStm.payload := mySelLoH2dPopStm
//  //  }
//  //}
//
//
...
//  //)
//  if (cfg.havePrefetch) {
//    io.haveHit.valid := tempToSwitchNonHaveHit.andR
//    io.haveHit.haveHitAtAll := haveHit.orR
//    io.haveHit.addr := rDel2LoH2dPayload.addr
//  }
//
//  switch (
//    tempToSwitch
...
private[libcheesevoyage] case class LcvBusInstrCacheNonWide(
  cfg: LcvBusCacheBusPairConfig
) extends Component {
  //--------
  val io = LcvBusCacheIo(cfg=cfg)
  val icache = (
    LcvBusInstrCacheMain(cfg=cfg)
    //LcvBusInstrCacheNoPrefetch(cfg=cfg)
  )
  io <> icache.io
  ////--------
  ...
  //  cfg.havePrefetch
  //) generate new Area {
  //  val icache = LcvBusInstrCacheWithPrefetch(cfg=cfg)
  //  io <> icache.io
  //}
  //val myDontSupportPrefetchArea = (
  //  !cfg.havePrefetch
  //) generate new Area {
  //  val icache = LcvBusInstrCacheNoPrefetch(cfg=cfg)
  //  io <> icache.io
  //}
}

case class LcvBusInstrCacheWide(
  cfg: LcvBusCacheBusPairConfig,
) extends Component {
  //--------
  def numWays = cfg.loBusCacheCfg.numWays
  ...
  def loBusCacheCfg = cfg.loBusCacheCfg
  def myLineWordRamAddrRshift = (
    loBusCacheCfg.myLineWordRamFullLineAddrRshift
  )
  def myLineAttrsRamAddrRshift = loBusCacheCfg.myLineAttrsRamAddrRshift
  def myLineWordRamArrFromBusAddrRange = (
    loBusCacheCfg.myLineWordRamArrFromBusAddrRange._1
    downto loBusCacheCfg.myLineWordRamArrFromBusAddrRange._2
  )

  def wordWidth = loBusCacheCfg.wordWidth
  def depthWords = loBusCacheCfg.depthWords
  def depthLines = loBusCacheCfg.depthLines
  def lineSizeWords = loBusCacheCfg.lineSizeWords
  ...
  for (idx <- 0 until numWays) {
    for (jdx <- 0 until lineSizeWords) {
      rdLineWord(idx)(jdx) := lineWordRam(idx)(jdx).io.rdData
      lineWordRam(idx)(jdx).io.wrEn := False
    }
  ...
  def doLineWordRamReadSync(
    ...
    lineWordRam.foreach(outerItem => outerItem.foreach(item => {
      if (setEn == 1) {
        item.io.rdEn := True
      } else if (setEn == 2) {
        item.io.rdEn := (
    ...
    switch (busAddr(
      ...
      for (wordIdx <- 0 until lineSizeWords) {
        is (wordIdx) {
          ...
          myRam.io.wrAddr := (
            (busAddr(busAddr.high downto myLineWordRamAddrRshift))
            .resize(myRam.io.wrAddr.getWidth)
          )
          myRam.io.wrData := lineWord
          //byteEn match {
          //  case Some(byteEn) => {
          //    myRam.io.wrByteEn := byteEn.asBits
          //  }
          //  case None => {
          //    myRam.io.wrByteEn := (
          //      B(
          //        myRam.io.wrByteEn.getWidth bits,
          //        default -> True
          //      )
          //    )
          //  }
...
// 3 0---
  ...
  val rSavedWordIdx = (
    Reg(UInt(log2Up(lineSizeWords) bits))
    init(0x0)
  )
  //val myHistSavedRamIdx = (
  //  History(
  ...
  switch (rState) {
    ...
    is (State.LOAD_HIT_DO_STALL_PIPE_4) {
      rState := State.LOAD_HIT_DO_STALL_PIPE_3
      lineAttrsRam.foreach(item => item.io.rdEn := False)
      lineWordRam.foreach(item => item.foreach(_.io.rdEn := False))
      myLoD2hPushStm.valid := False
      mySelLoH2dPopStm.ready := False
    ...
    is (State.LOAD_HIT_DO_STALL_PIPE_2) {
      rState := State.LOAD_HIT_DO_STALL_PIPE_1
      lineAttrsRam.foreach(item => item.io.rdEn := False)
      lineWordRam.foreach(item => item.foreach(_.io.rdEn := True))
      myLoD2hPushStm.valid := False
      mySelLoH2dPopStm.ready := False
    }
    is (State.LOAD_HIT_DO_STALL_PIPE_1) {
...
private[libcheesevoyage] case class LcvBusDataCacheMain(
  ...
  val rSavedLoH2dSavedData = (
    Reg(UInt(loBusCfg.dataWidth bits))
    init(0x0)
  )
  val rSavedLoH2dPayload = (
    Reg(cloneOf(rLoH2dPayload))
...
// 3 0---
  ...
  switch (
    ...
    is (
      ...
      //// cache miss, and know for sure that the line isn't dirty
      // cache miss
      rLoState := LoState.RECV_LINE_FROM_HI_BUS_PIPE_1
    ...
    is (
      ...
      // cache miss, and the line is *possibly* dirty
      rLoState := LoState.MAYBE_DIRTY_RE_READ_ATTRS_PIPE_2
  ...
  switch (rLoState) {
    ...
    is (LoState.MAYBE_DIRTY_RE_READ_ATTRS_PIPE_2) {
      rLoState := LoState.MAYBE_DIRTY_RE_READ_ATTRS_PIPE_1
      lineAttrsRam.foreach(item => item.io.rdEn := False)
      doLineAttrsRamReadSync(
        busAddr=rSavedLoH2dPayload.addr,
        setEn=0
    ...
    is (LoState.MAYBE_DIRTY_RE_READ_ATTRS_PIPE_1) {
      rLoState := LoState.MAYBE_DIRTY_RE_READ_ATTRS
      lineAttrsRam.foreach(item => item.io.rdEn := True)
    }
    is (LoState.MAYBE_DIRTY_RE_READ_ATTRS) {
      val myRdLineAttrs = (
        if (myCondHaveLineBitPlruRam) (
          rdLineAttrs(rSavedRamIdx)
        ) else (
      ...
      when (myRdLineAttrs.fire) {
        rLoState := LoState.SEND_LINE_TO_HI_BUS_PIPE_3
      } otherwise {
        rLoState := LoState.RECV_LINE_FROM_HI_BUS_PIPE_1
      }
      lineAttrsRam.foreach(item => item.io.rdEn := False)
    ...
    is (LoState.SEND_LINE_TO_HI_BUS_PIPE_3) {
      rLoState := LoState.SEND_LINE_TO_HI_BUS_PIPE_2
      lineAttrsRam.foreach(item => item.io.rdEn := False)
      rHiH2dPayload.burstLast := False
    ...
    is (LoState.SEND_LINE_TO_HI_BUS_PIPE_2) {
      rLoState := LoState.SEND_LINE_TO_HI_BUS_PIPE_1
      lineAttrsRam.foreach(item => item.io.rdEn := False)
    ...
    is (LoState.SEND_LINE_TO_HI_BUS_PIPE_1) {
      val myRdLineWord = (
        if (myCondHaveLineBitPlruRam) (
          rdLineWord(rSavedRamIdx)
        ) else (
      ...
      rLoState := LoState.SEND_LINE_TO_HI_BUS
      lineAttrsRam.foreach(item => item.io.rdEn := False)
      doLineWordRamReadSync(
        busAddr=hiBusCfg.burstAddr(
          someAddr=(
    ...
    is (LoState.SEND_LINE_TO_HI_BUS) {
      val myRdLineWord = (
        if (myCondHaveLineBitPlruRam) (
          rdLineWord(rSavedRamIdx)
        ) else (
    ...
    is (LoState.RECV_LINE_FROM_HI_BUS_PIPE_1) {
      rHadHiH2dFinish := False
      rHadHiD2hFinish := False
      ...
      when (RegNext(!hiH2dFifo.io.occupancy.orR, init=False)) {
        rHiH2dValid := True
        rLoState := LoState.RECV_LINE_FROM_HI_BUS
      }
    ...
    is (LoState.RECV_LINE_FROM_HI_BUS) {
      lineAttrsRam.foreach(item => item.io.rdEn := False)
      lineWordRam.foreach(item => item.io.rdEn := False)
      rHiH2dValid := False
      when (io.hiBus.d2hBus.valid) {
      ...
      when (io.hiBus.d2hBus.fire) {
        ...
        when ((io.hiBus.d2hBus.burstLast)) {
          when (!rSavedLoH2dPayload.isWrite) {
            rLoState := LoState.RECV_LINE_FROM_HI_BUS_POST_7
          } otherwise {
            rLoState := LoState.RECV_LINE_FROM_HI_BUS_POST_WRITE
          }
        }
      }
    }
    is (LoState.RECV_LINE_FROM_HI_BUS_POST_WRITE) {
      lineAttrsRam.foreach(item => item.io.rdEn := False)
      lineWordRam.foreach(item => item.io.rdEn := False)
    ...
    is (LoState.RECV_LINE_FROM_HI_BUS_POST_7) {
      lineWordRam.foreach(item => item.io.rdEn := False)
      lineAttrsRam.foreach(item => item.io.rdEn := False)
      rLoState := LoState.RECV_LINE_FROM_HI_BUS_POST_6
    }
    is (LoState.RECV_LINE_FROM_HI_BUS_POST_6) {
      lineWordRam.foreach(item => item.io.rdEn := False)
      lineAttrsRam.foreach(item => item.io.rdEn := False)
      rLoState := LoState.RECV_LINE_FROM_HI_BUS_POST_5
    }
    is (LoState.RECV_LINE_FROM_HI_BUS_POST_5) {
      lineWordRam.foreach(item => item.io.rdEn := False)
      lineAttrsRam.foreach(item => item.io.rdEn := False)
      rLoState := LoState.RECV_LINE_FROM_HI_BUS_POST_4
    }
    is (LoState.RECV_LINE_FROM_HI_BUS_POST_4) {
      lineAttrsRam.foreach(item => item.io.rdEn := False)
      lineWordRam.foreach(item => item.io.rdEn := False)
      doLineWordRamReadSync(
        busAddr=rSavedLoH2dPayload.addr,
      ...
      rLoState := LoState.RECV_LINE_FROM_HI_BUS_POST_3
    }
    is (LoState.RECV_LINE_FROM_HI_BUS_POST_3) {
      lineAttrsRam.foreach(item => item.io.rdEn := False)
      lineWordRam.foreach(item => item.io.rdEn := True)
      rLoState := LoState.RECV_LINE_FROM_HI_BUS_POST_2
    }
    is (LoState.RECV_LINE_FROM_HI_BUS_POST_2) {
      lineAttrsRam.foreach(item => item.io.rdEn := False)
      lineWordRam.foreach(item => item.io.rdEn := False)
      rLoState := LoState.RECV_LINE_FROM_HI_BUS_POST_1
    }
    is (LoState.RECV_LINE_FROM_HI_BUS_POST_1) {
      val myRdLineWord = (
        if (myCondHaveLineBitPlruRam) (
          rdLineWord(rSavedRamIdx)
        ) else (
      ...
      when (RegNext(
        rose(rLoState === LoState.RECV_LINE_FROM_HI_BUS_POST_2)
      )) {
        myLoD2hPushStm.busPayload.data := RegNext(
          //lineWordRam.io.rdData
          myRdLineWord
      ...
      when (myLoD2hPushStm.ready) {
        rLoState := LoState.RECV_LINE_FROM_HI_BUS_POST
      }
    }
    is (LoState.RECV_LINE_FROM_HI_BUS_POST) {
      lineAttrsRam.foreach(item => item.io.rdEn := False)
      lineWordRam.foreach(item => item.io.rdEn := False)
      rLoState := (
        LoState.IDLE
...
private[libcheesevoyage] case class LcvBusDataCacheNonWide(
  cfg: LcvBusCacheBusPairConfig
) extends Component {
  //--------
  val io = LcvBusCacheIo(cfg=cfg)
  ...
  //  cfg.havePrefetch
  //) generate new Area {
  //  val icache = LcvBusDataCacheWithPrefetch(cfg=cfg)
  //  io <> icache.io
  //}
  //val myDontSupportPrefetchArea = (
  //  !cfg.havePrefetch
  //) generate new Area {
  //  val icache = LcvBusDataCacheNoPrefetch(cfg=cfg)
  //  io <> icache.io
  //}
}
...
//        rSavedLoH2dPayload := rDel2LoH2dPayload
//        myLoD2hPayload.src := rDel2LoH2dPayload.src
//        base.myLoD2hStm.busPayload.txnCnt := (
//          base.rDel2LoH2dPayload.busPayload.txnCnt
//        )
//      }
...
case class LcvBusCache(
  ...
  val instrCache = (
    ...
    LcvBusInstrCacheNonWide(cfg=cfg)
  )
  val dataCache = (
    //cfg.haveNonCoherentDataCache
    cfg.loBusCacheCfg.kind == LcvCacheKind.D
  ) generate (
    LcvBusDataCacheNonWide(cfg=cfg)
  )
  ...
  val cacheCfg = (
    LcvBusCacheBusPairConfig(
      mainCfg=LcvBusMainConfig(
        ...
        haveByteEn=false,
        keepByteSize=false,
        //optTxnCntWidth=Some(LcvBusDoStallFifoThing.myCntWidth)
      ),
      loBusCacheCfg=LcvBusCacheConfig(
        kind=LcvCacheKind.D,
        lineSizeBytes=64,
